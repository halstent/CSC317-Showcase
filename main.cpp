...
int main(int argc, char* argv[])
{
    ...
    {
        std::cerr << "Exception inside read_json: " << e.what() << std::endl;
        return EXIT_FAILURE;
    }
    ...
    Eigen::SparseMatrix<double> M, A, C, K;
    Eigen::SimplicialLLT <Eigen::SparseMatrix<double > > prefactorization;
    if (!fast_mass_springs_precomputation_sparse(
        V, E, k, m, b, delta_t, r, M, A, C, K, prefactorization))
    {
        std::cerr << "precomputation failed." << std::endl;
    }

    // add colours to the vertices
    Eigen::MatrixXd CM = Eigen::MatrixXd::Zero(V.rows(), V.cols());

	// default color is black
    const Eigen::RowVector3d default_color(0.0, 0.0, 0.0);
	for (int i = 0; i < V.rows(); i++) { CM.row(i) = default_color; }

	// set boundary color to silver
    const Eigen::RowVector3d silver(0.8, 0.8, 0.8);
    for (int i = 0; i < b.rows(); i++) { CM.row(b(i)) = silver; }

	// set driver color to red
    const Eigen::RowVector3d driven_color(8.0, 6.0, 0.0);
    for (const Driver& d : drivers) { CM.row(d.vertex) = driven_color; }
    ...
    const auto step = [&]()
        {
            ...
            fast_mass_springs_step_sparse(
                V, E, b, delta_t, t, fext,
                r, M, A, C, K, prefactorization,
                Uprev, U_stash,
                U);
            Uprev = U_stash;
